...
- **Очікування без `sleep`.** Дії Playwright самі чекають, доки елемент стане видимим і доступним. Перевірки йдуть через `PlaywrightAssertions`, які повторюються до таймауту (10 с). Там, де стан змінюється асинхронно, чекаємо його явно: наприклад, що календар закрився після вибору дати, або що поле автокомпліту отримало фокус і введений текст, перш ніж обирати опцію. Навігація має окремий таймаут 45 с, бо demoqa віддає сторінку повільно (4–8 с навіть на один браузер), а на дії лишається 15 с.
